...
1. Цель инкремента
- Сохранить текущую UX-модель `start/end` assignment.
- Добавить внутри диапазона профиль загрузки сотрудника (не только константа), совместимый с day/month режимами.

2. Модель данных (этап 1 — без сложной математики)
- Ввести внутренние `key points` для assignment: `(date, value%)`.
- На MVP использовать piecewise-linear интерполяцию между точками (без bezier/tangents).
- Ограничения: `value` в диапазоне `0..100`, даты точек внутри assignment-диапазона.
- Начальная и конечная точки кривой могут иметь любое значение в диапазоне `0..100` (включая `0` и `100`).
- Добавить флаг режима профиля: `flat | curve` (для обратной совместимости).

3. Backend/API
- Расширить DTO assignment полем профиля загрузки (`loadProfile`).
- Добавить серверную валидацию:
   - минимум 2 точки для `curve`,
   - строго возрастающие даты точек,
   - точки не выходят за `assignment.start/end`.
- В `ProjectDetail` и timeline-response вернуть нормализованный профиль для UI.
- Зафиксировать error-коды для невалидного профиля (единый стиль с `ERR_*`).
...
- При resize assignment пересчитывать даты внутренних точек пропорционально новой длине диапазона (с сохранением относительных позиций точек).
- В timeline использовать дневное/месячное семплирование профиля и существующий рендер.
- При отсутствии профиля поведение полностью как сейчас (flat).
...
6. Этап 2 (после MVP)
- Добавить Unity-подобный curve editor с ручками (Bezier/tangents).
- Поддержать пресеты кривых (`ease-in`, `ease-out`, `plateau`, `ramp`).
...
- [x] Frontend UX: карточки проектов сделаны квадратными; создание проекта перенесено в пустую `+` карточку с auto-name `unnamed + date-time`.
...
- [x] Frontend/Backend: owner-only удаление проекта из project pop-up с подтверждением через ввод слова `delete`.
