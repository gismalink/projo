...
- [ ] Подключить источник производственного календаря через API: https://calendar.kuzyak.in/swagger.
- [ ] Реализовать backend-адаптер календаря (frontend не ходит во внешний API напрямую).
- [ ] Ввести хранение календарных дней (`CalendarDay`) с полями: `date`, `isWeekend`, `isHoliday`, `isWorkingDay`, `holidayName?`.
- [ ] Добавить кэширование календаря: prefetch текущего/следующего года, TTL 24ч, fallback на последний успешный снимок.
- [ ] Добавить расписание обновления календаря: ежедневный sync + ручной admin refresh + auto-refresh при смене года.
- [ ] Добавить UI-подсветку выходных/праздников на timeline и в графиках загрузки.
- [ ] Добавить в tooltip дня тип дня (рабочий/выходной/праздник) и название праздника.
- [ ] Пересчитать фактическое время assignment с учетом выходных, праздников и отпусков (day-level расчет).
- [ ] Добавить конфиг `standardDayHours` (дефолт 8ч) как базу для расчета фактических часов.
- [ ] Вывести `факт, ч` рядом с нагрузкой в строке assignment на timeline.
- [ ] Добавить агрегат фактических часов в проектных итогах (`project details` / summary).
- [ ] Добавить метрику «потерянные часы» (`planned - actual`) в строке assignment и/или агрегате проекта.
- [ ] Добавить легенду цветов типов дня (рабочий/выходной/праздник/отпуск).
- [ ] Добавить переключатель сценария «учитывать/не учитывать производственный календарь» для сравнения план/факт.
- [ ] Добавить health-метрику календаря: дата/время последнего успешного sync и статус актуальности.

Открытые решения:
- [ ] Что считать «видимым диапазоном» для `факт, ч`: год / фильтр timeline / assignment-диапазон.
- [ ] Нужны ли разные календари по стране/локации/офису.
- [ ] Что делать при частичном рабочем дне (если API это поддерживает).
