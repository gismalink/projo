...
6. [x] Добавить кнопку «Создать шаблонный проект» (seed demo-данных):
   - [x] создать 10 сотрудников с ролями и именами вида `Дизайнер Дизайнерски`, `Моделлер Моделлерски`, `Юнити Юнитевски`, `Юнити Грантовски`, `Веб Вебовски`, `Бэк Бэковски`, `Анали Аналитовски`, `Текст Текстисовски`, `Руководски`, `Тестировски`,
   - [x] назначить каждому отдел и грейд,
   - [x] создать 2–3 проекта,
   - [x] распределить сотрудников по проектам и планам (members/assignments).
7. [x] Компания не должна «пропадать» после удаления последнего проекта/плана (company может иметь 0 планов).
8. [x] Устранить ложные 429 `Too Many Requests` из-за неверных единиц `ttl` в throttler (секунды vs миллисекунды).
9. [ ] Добавить предварительную валидацию полей на фронте (до отправки формы на API).
   - [x] Авторизация (`login/register`): email format + min длины полей до API-запроса.
10. [x] В попапе авторизации центрировать кнопки «Войти» и «Зарегистрировать».
11. [x] На скамейке у выделенного сотрудника показывать «галочку» для явного отличия выбранного состояния от hover.
12. [x] Для выделенного сотрудника в timeline показывать только проекты, в которых он участвует, и только его самого в этих проектах (полная фильтрация связанных данных).
13. [x] Добавить выделение целого отдела на скамейке с фильтрацией timeline по выбранному отделу.
...
14. [ ] Подготовить следующий пакет продуктовых улучшений (отчеты, фильтры, интеграционные API-контракты).
