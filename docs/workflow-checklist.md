...
Правило итераций:
- Если для проекта настроен локальный dev и изменение можно корректно проверить локально — обновлять `test` на каждый шаг не обязательно.
- Для любых изменений, которые планируются к выкатыванию в `prod`, проверка на `test` обязательна.
